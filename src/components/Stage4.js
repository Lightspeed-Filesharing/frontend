...
const Stage4 = () => {
    ...
    useEffect(() => {
        setFullURL(
            `${window.origin}/${state.response.fileUuid}#${state.password}${state.salt}`
        );
        setDeleteURL(
            `${process.env.REACT_APP_API}/delete/${state.response.deletionUuid}`
        );
    }, []);
        ...
        <BottomStage4>
            <div className="bottom-child">
                ...
                <div className="center stage4">
                    <p className="direction" style={{ textAlign: "center" }}>
                        Your file is ready to be securely shared.
                    </p>
                    <p className="direction small">Copy your link below.</p>
                    <input
                        className="input stage4"
                        disabled={true}
                        value={fullURL}
                    />
                    ...
                    <button className="button create" onClick={resetStage}>
                        Share New File
                    </button>
                </div>
            </div>
